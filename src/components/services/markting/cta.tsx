...
export default function BrandingFinalCTA() {
    const t = useTranslations("DigitalMarketingService.BrandingFinalCTA");
    ...
    return (
        <section className="relative w-full py-10 border-t border-white/10 px-4 sm:px-6 md:px-12 select-none z-10 overflow-hidden" dir={currentDir}>
            <div className="relative z-10 w-full max-w-[1000px] mx-auto">
                ...
                >
                    {/* تفاصيل تقنية ومؤشرات واجهة المستخدم على الزوايا - تم تكييف أركانها شرطياً للتناظر الفخم */}
                    <div className={`absolute top-0 w-4 h-[1px] bg-white/20 group-hover:bg-blue-600/50 transition-colors duration-500 ${currentDir === "ltr" ? "left-0" : "right-0"}`} />
                    <div className={`absolute top-0 h-4 w-[1px] bg-white/20 group-hover:bg-blue-600/50 transition-colors duration-500 ${currentDir === "ltr" ? "left-0" : "right-0"}`} />
                    <div className={`absolute bottom-0 w-4 h-[1px] bg-white/10 group-hover:bg-blue-600/30 transition-colors duration-500 ${currentDir === "ltr" ? "right-0" : "left-0"}`} />
                    <div className={`absolute bottom-0 h-4 w-[1px] bg-white/10 group-hover:bg-blue-600/30 transition-colors duration-500 ${currentDir === "ltr" ? "right-0" : "left-0"}`} />
                    ...
                    <div className="mb-6 sm:mb-8 flex items-center gap-2 text-[9px] sm:text-[10px] text-blue-600 tracking-widest bg-blue-600/5 border border-blue-600/10 px-3 py-1 rounded-full uppercase">
                        ...
                        <span className="text-blue-600">{t("systemBadge")}</span>
                    </div>
                    ...
                    >
                        {/* ضبط الـ Padding التناظري للاتجاهين ليحافظ على الهيكل الهندسي متزن بالملي */}
                        <Link
                            href="/contact"
                            className={`group/btn inline-flex items-center gap-4 py-3 bg-white text-black hover:bg-blue-600 hover:text-white font-bold text-sm sm:text-base rounded-full transition-colors duration-300 ease-in-out tracking-tight shadow-sm ${currentDir === "rtl"
                                    ? "pr-8 pl-4 sm:pr-10 sm:pl-5 sm:py-4"
                                    : "pl-8 pr-4 sm:pl-10 sm:pr-5 sm:py-4"
                                }`}
                        >
                            <span>{t("ctaText")}</span>

                            {/* الدائرة والسهم: تم معالجة اتجاه حركة السهم بالـ Translate والـ Rotation ميكرونياً حسب اتجاه الصفحة الحالية */}

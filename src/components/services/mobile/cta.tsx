...
export default function MobileFinalCTA() {
    const t = useTranslations("MobileService.FinalCTA");
    ...
    return (
        <section className="relative w-full py-10 border-t border-white/10 px-4 sm:px-6 md:px-12 select-none z-10 overflow-hidden" dir={currentDir}>
            ...
            <div className="relative z-10 w-full max-w-[1000px] mx-auto">
                ...
                >
                    {/* تفاصيل تقنية ومؤشرات واجهة المستخدم على الزوايا لإتمام الهوية البرمجية الفخمة متناظرة الأطراف جغرافيا */}
                    ...
                    {/* ترويسة النظام الرقمي الخافتة - تم تحديثها للموبايل بنعومة */}
                    ...
                    >
                        <Link
                            href="/contact"
                            className={`group/btn inline-flex items-center bg-white text-black hover:bg-blue-600 hover:text-white font-bold text-sm sm:text-base rounded-full transition-colors duration-300 ease-in-out tracking-tight shadow-sm ${currentDir === "rtl"
                                ? "gap-4 pr-8 pl-4 py-3 sm:pr-10 sm:pl-5 sm:py-4"
                                : "gap-4 pl-8 pr-4 py-3 sm:pl-10 sm:pr-5 sm:py-4" // قلب الـ Padding للـ LTR لتأمين توازن أبعاد الدائرة داخل الكبسولة
                                }`}
                        >
                            <span>{t("ctaText")}</span>

                            {/* الدائرة: التحكم التناظري بالحركة بناءً على لغة تصفح الواجهة بالملي */}
                            <span className={`flex items-center justify-center w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-blue-600 text-white group-hover/btn:bg-white group-hover/btn:text-blue-600 transition-colors duration-300 ease-in-out shrink-0 ${currentDir === "ltr" ? "rotate-180" : ""
                                }`}>
                                <svg
                                    className={`w-4 h-4 transform transition-transform duration-300 ease-in-out ${currentDir === "rtl"
                                        ? "group-hover/btn:-translate-x-1.5"
                                        : "group-hover/btn:translate-x-1.5" // اندفاع لليمين فخم للإنجليزي تماشيًا مع الدوران
                                        }`}
                                    fill="none"
                                    viewBox="0 0 24 24"
                                    stroke="currentColor"
